function New-HtmlAudio {
   ...
   param(
      ...
      [Parameter()]
      [AngleSharp.Dom.Media.ITextTrackList]
      $TextTracks,
   ...
   begin {
    ...
    if ($IsSeeking  -eq $False) {  $null = $PSBoundParameters.Remove("IsSeeking") }
    if ($IsAutoplay  -eq $False) {  $null = $PSBoundParameters.Remove("IsAutoplay") }
    if ($IsLoop  -eq $False) {  $null = $PSBoundParameters.Remove("IsLoop") }
    if ($IsShowingControls  -eq $False) {  $null = $PSBoundParameters.Remove("IsShowingControls") }
    if ($IsDefaultMuted  -eq $False) {  $null = $PSBoundParameters.Remove("IsDefaultMuted") }
    if ($IsMuted  -eq $False) {  $null = $PSBoundParameters.Remove("IsMuted") }
   ...
   process {
    # Create HtmlElement
    $HtmlElement = $script:HtmlDocument.CreateElement('audio')
