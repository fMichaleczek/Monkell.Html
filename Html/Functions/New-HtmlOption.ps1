function New-HtmlOption {
   ...
   begin {
    ...
    if ($IsDefaultSelected  -eq $False) {  $null = $PSBoundParameters.Remove("IsDefaultSelected") }
    if ($IsSelected  -eq $False) {  $null = $PSBoundParameters.Remove("IsSelected") }
   ...
   process {
    # Create HtmlElement
    $HtmlElement = $script:HtmlDocument.CreateElement('option')
