function New-HtmlIframe {
   ...
   begin {
    ...
    if ($IsSeamless  -eq $False) {  $null = $PSBoundParameters.Remove("IsSeamless") }
    if ($IsFullscreenAllowed  -eq $False) {  $null = $PSBoundParameters.Remove("IsFullscreenAllowed") }
   ...
   process {
    # Create HtmlElement
    $HtmlElement = $script:HtmlDocument.CreateElement('iframe')
