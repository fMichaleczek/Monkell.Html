...
[CmdletBinding()]
    ...
    if ($Class.Count -gt 0) { $PSBoundParameters["ClassName"] = ($Class -join " ").Trim() } 
            $null = $PSBoundParameters.Remove('LayoutContent')  
 
            $null = $PSBoundParameters.Remove('LayoutSource')  
    ...
    try {
        ...
        $wrappedCmd = $ExecutionContext.InvokeCommand.GetCommand('New-HtmlQuote', [System.Management.Automation.CommandTypes]::Function)
